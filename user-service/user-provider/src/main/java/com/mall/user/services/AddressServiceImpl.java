...
public class AddressServiceImpl implements IAddressService {
     ...
     */
    @Override
    public AddressResponse addressUserName(AddressRequest request) {
        //TODO 地址信息要做缓存处理
        ...
        try{
            ...
            example.createCriteria().andEqualTo("userId", request.getUserId()).andEqualTo("addressId", request.getAddressId());
            List<Address> addresses = addressMapper.selectByExample(example);
            if (addresses.size() != 1 || addresses.get(0) == null) {
                log.error("AddressServiceImpl.addressList ");
                return null;
            }

            response.setUserName(addresses.get(0).getUserName());
            response.setCode(SysRetCodeConstants.SUCCESS.getCode());
            response.setMsg(SysRetCodeConstants.SUCCESS.getMessage());
        }catch (Exception e){
            log.error("AddressServiceImpl.addressList occur Exception :"+e);
